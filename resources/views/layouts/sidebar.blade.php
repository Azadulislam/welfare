...
<nav class="sidebar sidebar-offcanvas" id="sidebar">
    <ul class="nav">
        ...
        <li class="nav-item @if(in_array($route_name, $khairat)) active @endif @if(in_array($route_name, $death))) active @endif @if(in_array($route_name, $burial))) active @endif">
            ...
            <div class="collapse @if(in_array($route_name, $khairat)) show @endif @if(in_array($route_name, $death)) show @endif @if(in_array($route_name, $burial)) show @endif" id="memorial_services">
                <ul class="nav flex-column sub-menu">
                    <li class="nav-item  @if(in_array($route_name, $khairat)) active @endif"><a class="nav-link" href="{{ route('khairat.index') }}">
                        <img class="h-[20px] w-[20px] me-3" src="{{ asset('/images/add-user.svg') }}"/> Khairat Membership</a>
                    </li>
                    <li class="nav-item  @if(in_array($route_name, $death)) active @endif"><a class="nav-link" href="{{ route('death.index') }}">
                        <img class="h-[20px] w-[20px] me-3" src="{{ asset('/images/death.svg') }}"/>Death</a>
                    </li>
                    <li class="nav-item  @if(in_array($route_name, $burial)) active @endif"><a class="nav-link" href="{{ route('burial.payment.index') }}">
        ...
        <li class="nav-item @if(in_array($route_name, $welfareAll))) active @endif">
            ...
            <div class="collapse @if(in_array($route_name, $welfareAll)) show @endif" id="welfare_services">
                <ul class="nav flex-column sub-menu">
                    <li class="nav-item  @if(in_array($route_name, $welfareAll))  @if(!isset($category)) active @endif @endif"><a class="nav-link" href="{{ route('welfare.index') }}"><img
                                class="h-[20px] w-[20px] me-3" src="{{ asset('/images/grid-view-icon.svg') }}"/>All Category</a></li>
                    <li class="nav-item  @if(in_array($route_name, $welfareOrphan)) @if(isset($category) && $category == 'orphan') active @endif @endif"><a class="nav-link" href="{{ route('service', 'orphan') }}"><img
                                class="h-[20px] w-[20px] me-3" src="{{ asset('/images/orphan.svg') }}"/>Orphan</a></li>
                    <li class="nav-item  @if(in_array($route_name, $database)) active @endif"><a class="nav-link" href="{{ route('service', 'asnaf') }}"><img
                                class="h-[20px] w-[20px] me-3" src="{{ asset('/images/asnaf.svg') }}"/>Asnaf</a>
                    </li>
                    <li class="nav-item  @if(in_array($route_name, $database)) @if(isset($category) && $category == 'orphan') active @endif @endif"><a class="nav-link" href="{{ route('service', 'welfare') }}"><img
                                class="h-[20px] w-[20px] me-3" src="{{ asset('/images/welfare.svg') }}"/>Welfare</a>
                    </li>
                    <li class="nav-item  @if(in_array($route_name, $database)) active @endif"><a class="nav-link" href="{{ route('service', 'education') }}"><img
                                class="h-[20px] w-[20px] me-3" src="{{ asset('/images/education.svg') }}"/>Education</a>
                    </li>
                    <li class="nav-item  @if(in_array($route_name, $database)) active @endif"><a class="nav-link" href="{{ route('service', 'others') }}"><img
